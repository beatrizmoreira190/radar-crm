...
export default function PublisherDetailPage(){
  ...
  return <div className="page-wrap">
    ...
    <div className="detail-grid publisher-record-grid"><div className="detail-stack publisher-record-main">
      <section className="card panel publisher-data-card"><div className="section-title"><h2>Dados da editora</h2></div><div className="info-grid"><Info label="Razão social" value={publisher.legal_name} help={HELP.legalName}/><Info label="Nome fantasia" value={publisher.trade_name} help={HELP.tradeName}/><Info label="Nome comercial / marca" value={publisher.commercial_name} help="Nome pelo qual a editora se apresenta publicamente, identificado em fontes públicas quando o nome fantasia oficial não está disponível."/><Info label="CNPJ" value={publisher.cnpj} help={HELP.cnpj}/><Info label="Situação cadastral" value={publisher.registration_status} help="Situação atual do CNPJ informada pela Receita Federal."/><Info label="Data da situação cadastral" value={publisher.cnpj_status_date?formatDate(publisher.cnpj_status_date):null} help="Data associada à situação cadastral atual na base do CNPJ."/><Info label="Localização" value={[publisher.address_street,publisher.address_number,publisher.city,publisher.state].filter(Boolean).join(', ')} help={HELP.location}/><Info label="Perfil cadastral" value={publisher.profile} help={HELP.registeredProfile}/><Info label="Segmentos de atuação" value={(publisher.market_segments||[]).join(', ')} help={HELP.marketSegments}/><Info label="Porte" value={publisher.company_size||publisher.size_label} help={HELP.companySize}/></div>{cnpjVerification&&<div className="card" style={{marginTop:16,padding:14,boxShadow:'none',background:'#fafafa'}}><div style={{display:'flex',gap:10,alignItems:'flex-start'}}><DatabaseZap size={19} style={{marginTop:1,flex:'0 0 auto'}}/><div><strong style={{fontSize:12}}>Dados cadastrais conferidos com a Receita Federal</strong><p className="muted" style={{fontSize:11,margin:'4px 0 0'}}>Última verificação: <b>{dateTimeLabel(cnpjVerification.last_verified_at)}</b>{cnpjVerification.source_period?<><span> · Base </span><b>{cnpjVerification.source_period}</b></>:null}{cnpjVerification.status==='matched'?' · CNPJ localizado':cnpjVerification.status==='not_found'?' · CNPJ não localizado':' · Verificação com ocorrência'}</p></div></div></div>}{publisher.commercial_name&&<div className="card" style={{marginTop:10,padding:14,boxShadow:'none',background:'#fafafa'}}><div style={{display:'flex',gap:10,alignItems:'flex-start'}}><Globe2 size={19} style={{marginTop:1,flex:'0 0 auto'}}/><div><strong style={{fontSize:12}}>Enriquecimento por fontes públicas</strong><p className="muted" style={{fontSize:11,margin:'4px 0 0'}}>Nome comercial: <b>{publisher.commercial_name}</b>{publisher.commercial_name_confidence?<><span> · Confiança </span><b>{publisher.commercial_name_confidence==='high'?'alta':publisher.commercial_name_confidence==='medium'?'média':'baixa'}</b></>:null}{publisher.commercial_name_verified_at?<><span> · Verificado em </span><b>{dateTimeLabel(publisher.commercial_name_verified_at)}</b></>:null}</p>{Array.isArray(publisher.commercial_name_sources)&&publisher.commercial_name_sources.length>0&&<div className="publisher-meta" style={{marginTop:7}}>{publisher.commercial_name_sources.map((src,index)=>src?.url?<a className="text-link" href={src.url} target="_blank" rel="noreferrer" key={(src.url||'fonte')+'-'+index}><Globe2 size={12}/>{src.label||'Fonte pública'}</a>:null)}</div>}</div></div></div>}<div style={{marginTop:18,paddingTop:16,borderTop:'1px solid #f0f1f3'}}><div className="section-title" style={{marginBottom:9}}><HelpHeading help={HELP.editorialProfile}>Perfil editorial</HelpHeading><div className="chips"><span className={`badge ${publisher.editorial_profile_status==='confirmed'?'green':publisher.editorial_profile_status==='review'?'red':publisher.editorial_profile_status==='partial'?'amber':''}`}>{EDITORIAL_PROFILE_STATUS_LABELS[publisher.editorial_profile_status]||publisher.editorial_profile_status||'Pendente'}</span>{publisher.editorial_profile_confidence&&<span className="badge">Confiança {EDITORIAL_PROFILE_CONFIDENCE_LABELS[publisher.editorial_profile_confidence]||publisher.editorial_profile_confidence}</span>}</div></div>{publisher.editorial_profile?.length?<div className="chips">{publisher.editorial_profile.map(item=><span className="badge blue" key={item}>{item}</span>)}</div>:<p className="muted" style={{fontSize:12,margin:'4px 0'}}>{publisher.editorial_profile_status==='review'?'Cadastro marcado para revisão antes de classificar o perfil editorial.':publisher.editorial_profile_status==='not_identified'?'Não foi possível identificar o perfil editorial com segurança.':'Perfil editorial ainda não enriquecido.'}</p>}{publisher.editorial_profile_notes&&<p className="muted" style={{fontSize:11,margin:'10px 0 0'}}>{publisher.editorial_profile_notes}</p>}{editorialSources.length>0&&<div className="publisher-meta" style={{marginTop:9}}>{editorialSources.map((src,index)=>src?.url?<a className="text-link" href={src.url} target="_blank" rel="noreferrer" key={`${src.url}-${index}`}><Globe2 size={13}/>{src.label||'Fonte do perfil'}</a>:null)}{publisher.editorial_profile_verified_at&&<span>Verificado em {formatDate(publisher.editorial_profile_verified_at)}</span>}</div>}</div><div className="publisher-meta" style={{marginTop:16}}>{publisher.website&&<a className="text-link" href={publisher.website.startsWith('http')?publisher.website:`https://${publisher.website}`} target="_blank" rel="noreferrer"><Globe2 size={14}/>{publisher.website}</a>}{publisher.general_email&&<a className="text-link" href={`mailto:${publisher.general_email}`}><Mail size={14}/>{publisher.general_email}</a>}{publisher.phone&&<span><Phone size={13}/> {publisher.phone}</span>}</div></section>
