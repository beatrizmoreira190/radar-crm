...
export default function PublishersPage(){
  ...
  async function load(){
    ...
    let query=supabase.from('publishers').select('id,name,legal_name,trade_name,cnpj,city,state,priority,score,stage_id,owner_user_id,prospector_user_id,last_contact_at,next_action_at,commercial_temperature,editorial_profile,editorial_profile_status',{count:'exact'}).eq('organization_id',org).eq('archived',false);
    const q=filters.q.trim().replace(/[,%()]/g,' ');
    if(q)query=query.or(`name.ilike.%${q}%,legal_name.ilike.%${q}%,trade_name.ilike.%${q}%,cnpj.ilike.%${q}%`);
    ...
    if(filters.prospector==='mine')query=query.eq('prospector_user_id',user?.id); else if(filters.prospector)query=query.eq('prospector_user_id',filters.prospector);
  ...
  return <div className="page-wrap">
    ...
    <section className="card table-card"><table className="data-table"><thead><tr><th>Editora / identificação</th><th>Local</th><th>Perfil editorial</th><th>Etapa</th><th>Prioridade</th><th>Score</th><th>Responsável atual</th><th>Último contato</th></tr></thead><tbody>{rows.map(p=><tr key={p.id}><td><div className="cell-main"><span style={{fontSize:10,fontWeight:700,textTransform:'uppercase',letterSpacing:'.04em',color:'#667085'}}>{p.trade_name?'Nome fantasia':'Nome cadastrado'}</span><Link className="row-link" href={`/app/editoras/${p.id}`}>{p.trade_name||p.name}</Link><span><b>Razão social:</b> {p.legal_name||'Não informada'}</span><span><b>CNPJ:</b> {p.cnpj||'Não informado'}</span></div></td><td>{[p.city,p.state].filter(Boolean).join(' / ')||'—'}</td><td>{p.editorial_profile?.length?<div className="cell-main"><strong>{p.editorial_profile.slice(0,2).join(' · ')}</strong>{p.editorial_profile.length>2&&<span>+{p.editorial_profile.length-2} perfil{p.editorial_profile.length-2===1?'':'s'}</span>}</div>:p.editorial_profile_status==='review'?<span className="badge amber">Revisar</span>:<span className="muted">—</span>}</td><td><span className="badge">{stageMap[p.stage_id]?.name||'Sem etapa'}</span></td><td><span className={`badge ${p.priority==='urgent'||p.priority==='high'?'red':p.priority==='medium'?'amber':''}`}>{PRIORITY_LABELS[p.priority]||p.priority||'—'}</span></td><td><strong>{p.score??0}</strong></td><td>{p.owner_user_id===user?.id?<span className="badge green">Você</span>:p.owner_user_id?(teamMap[p.owner_user_id]?.full_name||teamMap[p.owner_user_id]?.email||'Equipe'):<button className="btn secondary small" onClick={()=>claim(p)}><UserCheck size={13}/> Assumir</button>}</td><td>{formatDate(p.last_contact_at,true)}</td></tr>)}</tbody></table>{!loading&&!rows.length&&<div className="table-empty"><Building2 size={30}/><p>Nenhuma editora encontrada com esses filtros.</p>{hasFilters&&<button type="button" className="btn secondary small" onClick={reset}>Limpar filtros</button>}</div>}{loading&&<div className="table-empty">Carregando editoras…</div>}<Pagination page={page} totalPages={totalPages} onChange={setPage}/></section>
