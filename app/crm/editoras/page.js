...
import { BRAZIL_STATES, EDITORIAL_PROFILE_OPTIONS, EDITORIAL_PROFILE_STATUS_LABELS, PRIORITY_LABELS, RADAR_PRODUCT_LABELS, formatDate } from '@/lib/constants';
import { BestProductBadge } from '@/components/RadarCompatibility';
...
const EMPTY_FILTERS = {q:'',stage:'',priority:'',state:'',owner:'',contact:'',scoreMin:'',editorialProfile:'',enrichment:'',bestProduct:'',fitProduct:'',fitMin:''};
...
export default function PublishersPage(){
  ...
  const setProductFit=(product,min)=>{setFilters(f=>({...f,fitProduct:product,fitMin:String(min)}));setPage(1);setActiveView('')};
  ...
  async function load(){
    ...
    let query=supabase.from('publishers').select('id,name,trade_name,cnpj,city,state,priority,score,radar_fit_score,commercial_potential_score,data_quality_score,best_product,fit_pnld_literario,fit_pnld_didatico,fit_pnld_tecnico_metodologico,fit_radar_licitacoes,fit_radar_oportunidades,score_version,stage_id,owner_user_id,last_contact_at,next_action_at,commercial_temperature,editorial_profile,editorial_profile_status',{count:'exact'}).eq('organization_id',org).eq('archived',false);
    ...
    if(filters.bestProduct)query=query.eq('best_product',filters.bestProduct);
    if(filters.fitProduct&&FIT_FIELDS[filters.fitProduct]&&filters.fitMin!=='')query=query.gte(FIT_FIELDS[filters.fitProduct],Number(filters.fitMin)||0);
  ...
  return <div className="page-wrap">
    <div className="page-head"><div><div className="eyebrow">Base comercial</div><h1>Editoras</h1><p>Encontre as contas com maior compatibilidade e veja qual produto da Radar faz mais sentido para cada abordagem.</p></div>{isManager&&<button className="btn" onClick={()=>setShowNew(true)}><Plus size={16}/> Nova editora</button>}</div>
    ...
    <div className="card panel" style={{marginBottom:14,paddingBottom:14}}>
      <div className="section-title" style={{marginBottom:10}}><div><h3>Visões salvas</h3><p className="muted">Salve combinações como “PNLD Literário 80+” e retome a mesma seleção depois.</p></div><button className="btn secondary small" onClick={()=>setShowSaveView(true)}><Bookmark size={14}/> Salvar visão</button></div>
    ...
    {moreFilters&&<div className="card panel" style={{marginBottom:16}}><div className="form-grid">
      <label>UF<select value={filters.state} onChange={e=>setF('state',e.target.value)}><option value="">Todos os estados</option>{BRAZIL_STATES.map(uf=><option key={uf}>{uf}</option>)}</select></label>
      <label>Perfil editorial<select value={filters.editorialProfile} onChange={e=>setF('editorialProfile',e.target.value)}><option value="">Todos os perfis</option>{editorialProfileOptions.map(item=><option value={item.value} key={item.value}>{item.value}{item.publisher_count===null?'':` (${item.publisher_count})`}</option>)}</select></label>
      <label>Responsável<select value={filters.owner} onChange={e=>setF('owner',e.target.value)}><option value="">Qualquer responsável</option><option value="mine">Minhas editoras</option><option value="unassigned">Sem responsável</option>{team.filter(m=>m.active).map(m=><option key={m.user_id} value={m.user_id}>{m.full_name||m.email}</option>)}</select></label>
      <label>Contato<select value={filters.contact} onChange={e=>setF('contact',e.target.value)}><option value="">Qualquer situação</option><option value="never">Nunca contatada</option><option value="contacted">Já contatada</option></select></label>
      <label>Enriquecimento<select value={filters.enrichment} onChange={e=>setF('enrichment',e.target.value)}><option value="">Qualquer status</option>{Object.entries(EDITORIAL_PROFILE_STATUS_LABELS).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label>
      <label>Radar Score mínimo<input className="input" type="number" min="0" max="100" value={filters.scoreMin} onChange={e=>setF('scoreMin',e.target.value)} placeholder="Ex.: 80"/></label>
      <label>Melhor oportunidade<select value={filters.bestProduct} onChange={e=>setF('bestProduct',e.target.value)}><option value="">Qualquer produto</option>{Object.entries(RADAR_PRODUCT_LABELS).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label>
      <label>Produto para filtrar<select value={filters.fitProduct} onChange={e=>setF('fitProduct',e.target.value)}><option value="">Selecione um produto</option>{Object.entries(RADAR_PRODUCT_LABELS).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label>
      <label>Fit mínimo<input className="input" type="number" min="0" max="100" disabled={!filters.fitProduct} value={filters.fitMin} onChange={e=>setF('fitMin',e.target.value)} placeholder="Ex.: 80"/></label>
    </div></div>}

    <div className="chips" style={{marginBottom:12}}><button className={`chip ${filters.owner==='mine'?'active':''}`} onClick={()=>setF('owner',filters.owner==='mine'?'':'mine')}>Minha carteira</button><button className={`chip ${filters.owner==='unassigned'?'active':''}`} onClick={()=>setF('owner',filters.owner==='unassigned'?'':'unassigned')}>Sem responsável</button><button className={`chip ${filters.contact==='never'?'active':''}`} onClick={()=>setF('contact',filters.contact==='never'?'':'never')}>Nunca contatadas</button><button className={`chip ${filters.fitProduct==='pnld_literario'&&filters.fitMin==='80'?'active':''}`} onClick={()=>setProductFit('pnld_literario',80)}>PNLD Literário 80+</button><button className={`chip ${filters.fitProduct==='radar_licitacoes'&&filters.fitMin==='80'?'active':''}`} onClick={()=>setProductFit('radar_licitacoes',80)}>Licitações 80+</button><button className={`chip ${filters.fitProduct==='radar_oportunidades'&&filters.fitMin==='80'?'active':''}`} onClick={()=>setProductFit('radar_oportunidades',80)}>Oportunidades 80+</button><span className="badge">{count.toLocaleString('pt-BR')} resultado{count===1?'':'s'}</span></div>

    <section className="card table-card" style={{overflowX:'auto'}}><table className="data-table" style={{minWidth:1050}}><thead><tr><th>Editora</th><th>Local</th><th>Perfil editorial</th><th>Etapa</th><th>Prioridade</th><th>Compatibilidade Radar</th><th>Responsável</th><th>Último contato</th></tr></thead><tbody>{rows.map(p=><tr key={p.id}><td><div className="cell-main"><Link className="row-link" href={`/app/editoras/${p.id}`}>{p.name}</Link><span>{p.trade_name||p.cnpj||'—'}</span></div></td><td>{[p.city,p.state].filter(Boolean).join(' / ')||'—'}</td><td>{p.editorial_profile?.length?<div className="cell-main"><strong>{p.editorial_profile.slice(0,2).join(' · ')}</strong>{p.editorial_profile.length>2&&<span>+{p.editorial_profile.length-2} perfil{p.editorial_profile.length-2===1?'':'s'}</span>}</div>:p.editorial_profile_status==='review'?<span className="badge amber">Revisar</span>:<span className="muted">—</span>}</td><td><span className="badge">{stageMap[p.stage_id]?.name||'Sem etapa'}</span></td><td><span className={`badge ${p.priority==='urgent'||p.priority==='high'?'red':p.priority==='medium'?'amber':''}`}>{PRIORITY_LABELS[p.priority]||p.priority||'—'}</span></td><td><div className="cell-main"><strong>Radar Score {p.score??0}</strong><span style={{marginTop:5}}><BestProductBadge publisher={p} compact/></span></div></td><td>{p.owner_user_id===user?.id?<span className="badge green">Você</span>:p.owner_user_id?(teamMap[p.owner_user_id]?.full_name||teamMap[p.owner_user_id]?.email||'Equipe'):<button className="btn secondary small" onClick={()=>claim(p)}><UserCheck size={13}/> Assumir</button>}</td><td>{formatDate(p.last_contact_at,true)}</td></tr>)}</tbody></table>{!loading&&!rows.length&&<div className="table-empty"><Building2 size={30}/><p>Nenhuma editora encontrada com esses filtros.</p></div>}{loading&&<div className="table-empty">Carregando editoras…</div>}<Pagination page={page} totalPages={totalPages} onChange={setPage}/></section>
