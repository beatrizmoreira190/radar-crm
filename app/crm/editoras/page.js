...
import { BRAZIL_STATES, PRIORITY_LABELS, formatDate } from '@/lib/constants';

const PAGE_SIZE = 40;
const EMPTY_FILTERS = {q:'',stage:'',priority:'',state:'',owner:'',contact:'',scoreMin:''};
...
export default function PublishersPage(){
  ...
  async function load(){
    ...
    let query=supabase.from('publishers').select('id,name,trade_name,cnpj,city,state,priority,score,stage_id,owner_user_id,last_contact_at,next_action_at,commercial_temperature',{count:'exact'}).eq('organization_id',org).eq('archived',false);
  ...
  return <div className="page-wrap">
    <div className="page-head"><div><div className="eyebrow">Base comercial</div><h1>Editoras</h1><p>Encontre contas por perfil comercial, salve suas combinações de filtros e organize sua carteira.</p></div>{isManager&&<button className="btn" onClick={()=>setShowNew(true)}><Plus size={16}/> Nova editora</button>}</div>
    ...
    <div className="card panel" style={{marginBottom:14,paddingBottom:14}}>
      <div className="section-title" style={{marginBottom:10}}><div><h3>Visões salvas</h3><p className="muted">Salve filtros que você usa com frequência, como “Minhas editoras de SP” ou “Sem contato · score 80+”.</p></div><button className="btn secondary small" onClick={()=>setShowSaveView(true)}><Bookmark size={14}/> Salvar visão</button></div>
