...
export default function PublishersPage(){
  ...
  async function load(){
    ...
    let query=supabase.from('publishers').select('id,name,legal_name,trade_name,commercial_name,cnpj,city,state,priority,score,stage_id,owner_user_id,prospector_user_id,last_contact_at,next_action_at,commercial_temperature,editorial_profile,editorial_profile_status',{count:'exact'}).eq('organization_id',org).eq('archived',false);
    const q=filters.q.trim().replace(/[,%()]/g,' ');
    if(q)query=query.or(`name.ilike.%${q}%,commercial_name.ilike.%${q}%,legal_name.ilike.%${q}%,trade_name.ilike.%${q}%,cnpj.ilike.%${q}%`);
  ...
  return <div className="page-wrap">
    <div className="page-head"><div><div className="eyebrow">Base comercial</div><h1>Editoras</h1><p>Encontre contas por perfil editorial, etapa, localização e responsável para organizar sua prospecção.</p></div><Link className="btn" href="/app/editoras/nova"><Plus size={16}/> Nova editora</Link></div>
