      ...
      <div className="team-compact-list">{visibleTeam.map(m=>{
        ...
        return <article className={`card team-member-row ${expanded?'expanded':''}`} key={m.user_id}>
          <div className="team-member-summary">
            ...
            <button type="button" className="team-member-identity team-member-profile-link" onClick={()=>setPerformanceUser(m.user_id)} title="Ver desempenho individual"><strong>{m.full_name||m.email||'Usuário'}</strong><span>{m.email||''}</span><small>{m.job_title||ROLE_LABELS[m.role]||m.role}</small></button>
