...
export default function CrmHome(){
  ...
  return <div className="page-wrap">
    <div className="page-head"><div><div className="eyebrow">Visão geral</div><h1>Bom trabalho{membership?.full_name?`, ${membership.full_name.split(' ')[0]}`:''}.</h1><p>{isManager?'Veja as prioridades do dia, o andamento da equipe e os principais indicadores da operação.':'Veja suas prioridades do dia, tarefas, responsabilidade atual e movimentações recentes.'}</p></div><Link href="/app/editoras" className="btn">Abrir editoras <ArrowRight size={17}/></Link></div>
    <div className="metric-grid">
      <div className="metric-card"><div className="metric-icon"><Building2/></div><div><span>{isManager?'Editoras':'Responsabilidade atual'}</span><strong>{metrics.publishers.toLocaleString('pt-BR')}</strong><small>{isManager?'base ativa':'editoras que você conduz agora'}</small></div></div>
      <div className="metric-card"><div className="metric-icon"><PhoneCall/></div><div><span>Já contatadas</span><strong>{metrics.contacted.toLocaleString('pt-BR')}</strong><small>{metrics.publishers?Math.round(metrics.contacted/metrics.publishers*100):0}% {isManager?'da base':'da responsabilidade atual'}</small></div></div>
    ...
    <div className="dashboard-grid">
      ...
      <section className="card panel"><div className="panel-head"><div><h2>{isManager?'Maiores Radar Scores':'Maiores Radar Scores sob minha responsabilidade'}</h2><p>Score alto indica potencial; a ordem de execução fica em Prioridades.</p></div><Flame size={20}/></div><div className="hot-list">{hot.map(p=><Link href={`/app/editoras/${p.id}`} key={p.id} className="hot-row"><div><strong>{p.name}</strong><span>{[p.city,p.state].filter(Boolean).join(' / ')||'—'}</span></div><div className="score-pill">{p.score??0}</div></Link>)}</div><Link href="/app/prioridades" className="panel-link">Ver fila de Prioridades <ArrowRight size={15}/></Link></section>
      <section className="card panel span-2"><div className="panel-head"><div><h2>Pipeline</h2><p>{isManager?'Distribuição atual das editoras':'Distribuição atual das editoras sob sua responsabilidade'}</p></div><Link href="/app/pipeline" className="text-link">Explorar</Link></div><div className="pipeline-mini">{stages.map(s=>{const count=stageCounts[s.id]||0;const max=Math.max(...Object.values(stageCounts),1);return <div className="pipeline-mini-row" key={s.id}><span>{s.name}</span><div className="bar-track"><div className="bar-fill" style={{width:`${Math.max(2,count/max*100)}%`}}/></div><strong>{count}</strong></div>})}</div></section>
