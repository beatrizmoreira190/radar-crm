...
export default function PipelinePage(){
  const {supabase,membership,user,activityVersion,isManager}=useCrm(); const org=membership?.organization_id;
  ...
  async function load(){if(!org)return;setLoading(true);let publisherQuery=supabase.from('publishers').select('id,name,city,state,score,priority,stage_id,last_contact_at,next_action_at').eq('organization_id',org).eq('archived',false);if(!isManager)publisherQuery=publisherQuery.eq('owner_user_id',user?.id);const [{data:ss,error:se},{data:ps,error:pe}]=await Promise.all([supabase.from('pipeline_stages').select('id,name,position,stage_type').eq('organization_id',org).eq('active',true).order('position'),publisherQuery.order('score',{ascending:false,nullsFirst:false}).limit(1000)]);if(se||pe)setNotice(se?.message||pe?.message);setStages(ss||[]);setRows(ps||[]);setLoading(false)}
  ...
  return <div className="page-wrap"><div className="page-head"><div><div className="eyebrow">Funil comercial</div><h1>Pipeline</h1><p>{isManager?'Acompanhe a distribuição da equipe e mova as editoras conforme a prospecção avança.':'Acompanhe as editoras da sua carteira e mova cada conta conforme a prospecção avança.'}</p></div></div>{notice&&<div className="notice-bar"><span>{notice}</span><button onClick={()=>setNotice('')}><X size={15}/></button></div>}
