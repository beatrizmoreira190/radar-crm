  ...
  return <div className="page-wrap reports-v2">
    ...
    {loading?<div className="card table-empty">Calculando indicadores e comparativos…</div>:<>
      ...
      {view==='cadences'&&<>
        ...
        {(meetingAnalytics.by_presenter||[]).length>0&&<section className="card panel"><div className="panel-head"><div><h2>Reuniões por apresentador</h2><p>Volume e realização no período.</p></div><Users size={20}/></div><div className="report-table-wrap"><table className="report-table"><thead><tr><th>Apresentador</th><th>Reuniões</th><th>Realizadas</th><th>Taxa de realização</th></tr></thead><tbody>{meetingAnalytics.by_presenter.map(row=><tr key={row.user_id}><td>{isManager?<button type="button" className="report-person-link" onClick={()=>setPerformanceUser(row.user_id)}>{teamMap[row.user_id]?.full_name||teamMap[row.user_id]?.email||'Equipe'}</button>:teamMap[row.user_id]?.full_name||teamMap[row.user_id]?.email||'Equipe'}</td><td>{Number(row.meetings||0)}</td><td>{Number(row.completed||0)}</td><td>{Number(row.meetings)?Math.round(Number(row.completed)/Number(row.meetings)*100):0}%</td></tr>)}</tbody></table></div></section>}
      </>}

      {view==='team'&&isManager&&<>
        <section className="card panel"><div className="panel-head"><div><h2>Cobertura e produtividade da equipe</h2><p>Origem das contas, responsabilidade atual, contatos, reuniões, oportunidades e pendências.</p></div><Users size={20}/></div><TeamTable data={teamPerformance} onUserClick={setPerformanceUser}/></section>
...
function TeamTable({data,onUserClick}){
  if(!data?.length)return <p className="muted">Ainda não há atividade suficiente da equipe.</p>;
  return <div className="report-table-wrap"><table className="report-table"><thead><tr><th>Pessoa</th><th>Originadas</th><th>Responsabilidade atual</th><th>Cobertura atual</th><th>Interações</th><th>Reuniões</th><th>Oportunidades</th><th>Atrasadas</th></tr></thead><tbody>{data.map(row=><tr key={row.user_id}><td><button type="button" className="report-person-link" onClick={()=>onUserClick?.(row.user_id)}>{row.full_name||row.email||'Equipe'}</button></td><td>{Number(row.originated_publishers||0).toLocaleString('pt-BR')}</td><td>{Number(row.current_responsibility||0).toLocaleString('pt-BR')}</td><td>{Number(row.current_responsibility)?Math.round(Number(row.contacted||0)/Number(row.current_responsibility)*100):0}%</td><td>{Number(row.interactions||0)}</td><td>{Number(row.meetings||0)}</td><td>{Number(row.opportunities||0)}</td><td>{Number(row.overdue||0)}</td></tr>)}</tbody></table></div>
}
