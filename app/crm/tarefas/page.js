...
export default function TasksPage(){
  ...
  async function load(){
    ...
    const [{data,error},{data:pubs}]=await Promise.all([query.order('due_at',{ascending:true,nullsFirst:false}).limit(250),supabase.from('publishers').select('id,name').eq('organization_id',org).eq('archived',false).order('name').limit(500)]);
    if(error)setNotice(error.message);setRows(data||[]);setPublishers(pubs||[]);await loadReminders();setLoading(false);
  }
