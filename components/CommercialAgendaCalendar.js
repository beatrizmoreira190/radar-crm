...
export default function CommercialAgendaCalendar(){
  ...
  const visibleDays=useMemo(()=>allDays.slice(0,viewDays),[allDays,viewDays]);
  ...
  const busyByDay=useMemo(()=>{
    ...
    for(const block of externalBusy){const start=new Date(block.start),end=new Date(block.end);if(Number.isNaN(start)||Number.isNaN(end))continue;for(const day of allDays){const ds=new Date(day);ds.setHours(0,0,0,0);const de=addDays(ds,1);if(start<de&&end>ds)map[dayKey(day)].push({start:start>ds?start:ds,end:end<de?end:de})}}
    return map;
  },[externalBusy,allDays]);
  ...
  function dayAllowed(day,userId){return presenterRule(userId).active_days.includes(day.getDay())}
  function dayStartAt(day,minutes){const d=new Date(day);d.setHours(Math.floor(minutes/60),minutes%60,0,0);return d}
  ...
  function openQuick(day,minutes=null){
    ...
    let startMinutes=minutes??timeToMinutes(rule.work_start);
    let start=dayStartAt(day,startMinutes);
  ...
  function changeQuickPresenter(presenterId){
    const rule=presenterRule(presenterId);const current=quick?.scheduled_start?new Date(quick.scheduled_start):now;
    setQuick(value=>({...value,presenter_user_id:presenterId,duration_minutes:rule.default_duration_minutes,scheduled_start:localInput(current)}));
  }
  ...
  function eventGeometry(meeting){
    const start=minuteOfDay(meeting.scheduled_start);const rawEnd=start+Number(meeting.duration_minutes||30);const visibleStart=clamp(start,calendarStartMinute,calendarEndMinute);const visibleEnd=clamp(rawEnd,calendarStartMinute,calendarEndMinute);
    return{top:(visibleStart-calendarStartMinute)*PX_PER_MINUTE,height:Math.max(24,(visibleEnd-visibleStart)*PX_PER_MINUTE)};
  }
  function presenterLane(meeting){
    if(presenterFilter!=='all')return{left:'4px',right:'4px'};
    const count=Math.max(1,presenterOptions.length);const index=Math.max(0,presenterOptions.findIndex(item=>item.user_id===meeting.presenter_user_id));const width=100/count;
    return{left:`calc(${index*width}% + 3px)`,width:`calc(${width}% - 6px)`};
  }

  return <div className="page-wrap agenda-v2">
    <header className="agenda-v2-head">
      <div><div className="eyebrow">Agenda comercial</div><h1>Agenda da equipe</h1><p>Horários, reuniões Radar e indisponibilidades externas na mesma linha do tempo.</p></div>
      <button className="btn secondary" type="button" onClick={refreshAll}><RefreshCw size={15}/> Atualizar</button>
    </header>
    ...
    <section className="agenda-control card">
      <div className="week-switcher"><button className="icon-btn" type="button" onClick={previousWeek}><ChevronLeft size={18}/></button><div><small>Semana</small><strong>{rangeLabel(weekStart,addDays(weekStart,viewDays-1))}</strong></div><button className="icon-btn" type="button" onClick={nextWeek}><ChevronRight size={18}/></button><button className="btn secondary small" type="button" onClick={currentWeek}>Hoje</button></div>
      <div className="agenda-control-filters">
        ...
        <div className="day-toggle"><button type="button" className={viewDays===5?'active':''} onClick={()=>setViewDays(5)}>5 dias</button><button type="button" className={viewDays===7?'active':''} onClick={()=>setViewDays(7)}>7 dias</button></div>
    ...
    {loading?<section className="card table-empty">Carregando agenda…</section>:<section className="calendar-shell card">
      <div className="calendar-header" style={{gridTemplateColumns:`58px repeat(${viewDays},minmax(150px,1fr))`}}><div></div>{visibleDays.map((day,index)=><button type="button" key={dayKey(day)} className={`day-heading ${sameLocalDay(day,now)?'today':''}`} onClick={()=>openQuick(day)} disabled={!canSchedule}><span>{DAY_NAMES[index]}</span><strong>{shortDate(day)}</strong>{sameLocalDay(day,now)&&<small>Hoje</small>}</button>)}</div>
      <div className="calendar-scroll">
        <div className="calendar-grid" style={{gridTemplateColumns:`58px repeat(${viewDays},minmax(150px,1fr))`,height:calendarHeight}}>
          ...
          {visibleDays.map(day=>{
            const key=dayKey(day);const rows=meetingsByDay[key]||[];const busyRows=presenterFilter==='all'?[]:(busyByDay[key]||[]);const activePresenter=presenterFilter!=='all'?presenterFilter:null;const rule=activePresenter?presenterRule(activePresenter):null;const allowed=!rule||rule.active_days.includes(day.getDay());const breakStart=rule?.break_start?timeToMinutes(rule.break_start):null;const breakEnd=rule?.break_end?timeToMinutes(rule.break_end):null;
            return <div className={`day-column ${allowed?'':'disabled-day'}`} key={key}>
              ...
              {slotMarks.map(minutes=><button aria-label={`Agendar ${DAY_NAMES[(day.getDay()+6)%7]} ${Math.floor(minutes/60)}:${String(minutes%60).padStart(2,'0')}`} className="slot-button" type="button" key={minutes} disabled={!canSchedule||!allowed} onClick={()=>openQuick(day,minutes)} style={{top:(minutes-calendarStartMinute)*PX_PER_MINUTE,height:SLOT_MINUTES*PX_PER_MINUTE}}/>)}
              ...
              {sameLocalDay(day,now)&&minuteOfDay(now)>=calendarStartMinute&&minuteOfDay(now)<=calendarEndMinute&&<div className="now-line" style={{top:(minuteOfDay(now)-calendarStartMinute)*PX_PER_MINUTE}}><i/></div>}
    ...
    <style jsx>{`
      .agenda-v2{max-width:none}.agenda-v2-head{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;margin-bottom:14px}.agenda-v2-head h1{margin:4px 0 5px}.agenda-v2-head p{margin:0;color:#667085}.agenda-control{position:sticky;top:0;z-index:15;padding:10px 12px;margin-bottom:7px;display:flex;align-items:center;justify-content:space-between;gap:12px;background:rgba(255,255,255,.96);backdrop-filter:blur(9px)}.week-switcher{display:flex;align-items:center;gap:6px;white-space:nowrap}.week-switcher>div{display:grid;text-align:center;min-width:125px}.week-switcher small{font-size:8px;text-transform:uppercase;letter-spacing:.06em;color:#98a2b3;font-weight:800}.week-switcher strong{font-size:11px;color:#344054}.agenda-control-filters{display:flex;align-items:center;gap:7px;flex-wrap:wrap;justify-content:flex-end}.agenda-control-filters select{min-width:140px}.agenda-control-filters .search-box{min-width:230px}.day-toggle{display:flex;background:#f2f4f7;border-radius:8px;padding:2px}.day-toggle button{border:0;background:transparent;padding:6px 8px;border-radius:6px;font-size:10px;font-weight:700;color:#667085;cursor:pointer}.day-toggle button.active{background:white;color:#101828;box-shadow:0 1px 2px rgba(16,24,40,.08)}.agenda-status-line,.agenda-rule-line{display:flex;align-items:center;justify-content:space-between;gap:12px;font-size:10px;color:#667085;padding:0 3px 8px}.agenda-status-line strong{color:#344054}.agenda-status-line span:last-child{display:flex;align-items:center;gap:4px}.agenda-status-line .syncing svg{animation:spin 1s linear infinite}.agenda-rule-line{justify-content:flex-start;padding-top:0}@keyframes spin{to{transform:rotate(360deg)}}
      .calendar-shell{padding:0;overflow:hidden}.calendar-header{display:grid;border-bottom:1px solid #e4e7ec;background:#fcfcfd;position:sticky;top:55px;z-index:10}.day-heading{border:0;border-left:1px solid #eaecf0;background:transparent;padding:9px 8px;display:flex;align-items:baseline;justify-content:center;gap:6px;cursor:pointer;color:#667085}.day-heading:hover{background:#f9fafb}.day-heading span{font-size:10px;font-weight:700}.day-heading strong{font-size:11px;color:#344054}.day-heading small{font-size:8px;text-transform:uppercase;background:#eef4ff;color:#3538cd;border-radius:999px;padding:2px 5px}.day-heading.today{background:#f8f9fc}.calendar-scroll{overflow:auto;max-height:calc(100vh - 255px);min-height:520px}.calendar-grid{display:grid;min-width:860px;position:relative}.time-gutter{position:relative;border-right:1px solid #eaecf0;background:#fcfcfd}.time-gutter span{position:absolute;right:8px;font-size:9px;color:#98a2b3}.day-column{position:relative;border-right:1px solid #eaecf0;background:#fff;overflow:hidden}.day-column:last-child{border-right:0}.hour-line{position:absolute;left:0;right:0;border-top:1px solid #f0f1f3;pointer-events:none}.slot-button{position:absolute;left:0;right:0;border:0;border-top:1px dotted rgba(234,236,240,.55);background:transparent;cursor:pointer;z-index:1}.slot-button:not(:disabled):hover{background:rgba(242,244,247,.62)}.slot-button:disabled{cursor:default}.break-block{position:absolute;left:0;right:0;background:repeating-linear-gradient(135deg,#f9fafb,#f9fafb 6px,#f2f4f7 6px,#f2f4f7 12px);border-top:1px dashed #d0d5dd;border-bottom:1px dashed #d0d5dd;z-index:2;display:grid;place-items:center;pointer-events:none}.break-block span{font-size:8px;text-transform:uppercase;color:#98a2b3;font-weight:800}.external-busy-block{position:absolute;left:4px;right:4px;border:1px dashed #cfd4dc;background:rgba(249,250,251,.96);border-radius:7px;padding:5px 6px;z-index:3;color:#667085;display:grid;align-content:start;gap:2px;overflow:hidden}.external-busy-block>span{font-size:9px;font-weight:800}.external-busy-block small{font-size:8px}.calendar-event{position:absolute;border:1px solid #c7d7fe;background:#eef4ff;border-radius:8px;padding:5px 6px;text-align:left;z-index:5;cursor:pointer;overflow:hidden;box-shadow:0 1px 2px rgba(16,24,40,.08);color:#344054}.calendar-event:hover{filter:brightness(.98);box-shadow:0 2px 5px rgba(16,24,40,.12)}.calendar-event.completed{background:#ecfdf3;border-color:#abefc6}.calendar-event.cancelled,.calendar-event.no_show{background:#f9fafb;border-color:#d0d5dd;opacity:.72}.calendar-event>div{display:flex;align-items:center;gap:5px}.calendar-event>div strong{font-size:9px}.calendar-event :global(.badge){font-size:7px;padding:1px 4px;margin-left:auto}.calendar-event>b{display:block;font-size:10px;line-height:1.2;margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.calendar-event>small{display:block;font-size:8px;color:#667085;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2px}.calendar-event>em{display:flex;align-items:center;gap:3px;font-style:normal;font-size:8px;color:#667085;margin-top:4px;white-space:nowrap;overflow:hidden}.now-line{position:absolute;left:0;right:0;border-top:1px solid #d92d20;z-index:8;pointer-events:none}.now-line i{position:absolute;width:7px;height:7px;border-radius:50%;background:#d92d20;left:-3px;top:-4px}.disabled-day{background:#f9fafb}.disabled-overlay{position:absolute;inset:0;background:rgba(249,250,251,.54);z-index:7;display:grid;place-items:start center;pointer-events:none}.disabled-overlay span{margin-top:12px;font-size:8px;text-transform:uppercase;color:#98a2b3;font-weight:800}.quick-agenda-modal{max-width:630px}.quick-publisher-search{margin:5px 0}.quick-rule{display:flex;align-items:flex-start;gap:7px;padding:9px;border-radius:8px;background:#f2f4f7;color:#475467;font-size:10px}
      @media(max-width:1100px){.agenda-control{align-items:flex-start;flex-direction:column}.agenda-control-filters{justify-content:flex-start;width:100%}.calendar-header{top:94px}.calendar-scroll{max-height:calc(100vh - 290px)}}@media(max-width:680px){.agenda-v2-head{align-items:flex-start;flex-direction:column}.agenda-control-filters{display:grid;grid-template-columns:1fr 1fr}.agenda-control-filters .search-box{grid-column:1/-1;min-width:0}.agenda-control-filters>.btn{grid-column:1/-1}.agenda-status-line{align-items:flex-start;flex-direction:column}.calendar-scroll{max-height:70vh}.calendar-header{top:150px}}
    `}</style>
  </div>;
}
