        ...
        return <article className="meeting-row" key={meeting.id}>
          ...
          <div className="meeting-main">
            ...
            <div className="meeting-actions">
              ...
              {canEdit&&meeting.status==='scheduled'&&<><button type="button" className="mini-action" onClick={()=>openEdit(meeting)}><RotateCcw size={12}/> Reagendar/editar</button><button type="button" className="mini-action success" onClick={()=>setPostMeeting(meeting)}><CheckCircle2 size={12}/> Realizada</button><details className="meeting-more-actions"><summary title="Mais ações" aria-label="Mais ações">•••</summary><div className="meeting-more-menu"><button type="button" onClick={()=>markNoShow(meeting)}><UserX size={12}/> Não compareceu</button><button type="button" className="danger" onClick={()=>cancelMeeting(meeting)}><Ban size={12}/> Cancelar</button></div></details></>}
...
function MeetingModal({supabase,org,publisherId,publisherName,user,team,presenters,contacts,meeting,prefill,participants,materials,canEditScheduling,onClose,onSaved}){
  ...
  return <ModalDialog title={editing?'Editar reunião':'Agendar reunião'} description={canEditScheduling?'O CRM aplica as regras do apresentador, verifica conflitos e prepara automaticamente a fila de tarefas.':`Você está atualizando a reunião como apresentador(a): ${presenterName}.`} onClose={onClose} onSubmit={save} busy={busy}>{error&&<div className="notice error" role="alert">{error}</div>}<div className="form-grid">
    ...
    <label>Data e horário<PtBrDateTimeField required disabled={!canEditScheduling} value={form.scheduled_start} onChange={value=>setForm(x=>({...x,scheduled_start:value}))} ariaLabel="Data e horário"/></label>
    ...
    {canEditScheduling&&<div className="span-2 invite-preview"><div className="invite-preview-head"><div><strong>Resumo interno do agendamento</strong><p>Observações e materiais ficam somente no CRM. O convite externo recebe apenas título institucional, horário, convidados, Google Meet e uma descrição breve.</p></div>{materials.length?<span className="badge green"><FileCheck2 size={11}/> {materials.length} material{materials.length===1?'':'is'} pronto{materials.length===1?'':'s'}</span>:<span className="badge amber"><FileWarning size={11}/> Sem material pronto</span>}</div><div className="preview-grid"><div><small>Reunião</small><b>{form.title||'Reunião comercial'}</b></div><div><small>Quando</small><b>{form.scheduled_start?`${formatDate(new Date(form.scheduled_start),true)} · ${form.duration_minutes} min`:'Defina data e horário'}</b></div><div><small>Apresentação</small><b>{presenterName}</b></div><div><small>Editora</small><b>{publisherName}</b></div></div><div className="preview-people"><small>Participantes</small><span>{selectedPeople.length?selectedPeople.map(p=>p.email?`${p.name} <${p.email}>`:p.name).join(' · '):'Nenhum participante selecionado'}</span></div>{materials.length>0&&<div className="preview-people"><small>Materiais prontos</small><span>{materials.slice(0,4).map(m=>m.title).join(' · ')}{materials.length>4?` +${materials.length-4}`:''}</span></div>}</div>}
  </div><div className="modal-actions"><button className="btn secondary" type="button" disabled={busy} onClick={onClose}>Cancelar</button><button className="btn" disabled={busy||(!editing&&!presenters.length)}>{busy?'Salvando…':editing?'Salvar reunião':'Agendar reunião'}</button></div><style jsx>{`
