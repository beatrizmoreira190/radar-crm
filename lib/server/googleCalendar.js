...
function sendRadarMeetingReminders_() {
  ...
  events.forEach(event => {
    ...
    const meetingTitle = String(event.getTitle() || 'Reunião comercial')
      .replace(/^Radar\\s*\\|\\s*/i, '')
      .trim();
    const subject = 'Lembrete da nossa reunião — Grupo Radar';
    const body = [
      'Olá!',
      '',
      'Passando para lembrar da nossa reunião agendada para ' + when + '.',
      meetingTitle ? 'Reunião: ' + meetingTitle : '',
      '',
      'Caso surja algum imprevisto ou você não consiga participar, pedimos que nos avise para que possamos reorganizar a agenda.',
    ...
    const htmlBody =
      '<p>Olá!</p>' +
      '<p>Passando para lembrar da nossa reunião agendada para <strong>' + escapeHtml_(when) + '</strong>.</p>' +
      (meetingTitle ? '<p><strong>Reunião:</strong> ' + escapeHtml_(meetingTitle) + '</p>' : '') +
      '<p>Caso surja algum imprevisto ou você não consiga participar, pedimos que nos avise para que possamos reorganizar a agenda.</p>' +
      '<p>Basta responder a este e-mail.</p>' +
      '<p>Até breve,<br><strong>Grupo Radar</strong></p>';
