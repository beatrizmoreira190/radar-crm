...
function getGoogleMeetUrl_(event) {
  try {
    const visibleText = [event.getLocation(), event.getDescription()].join('\\n');
    const direct = visibleText.match(/https:\/\/meet\\.google\\.com\/[a-z0-9-]+/i);
    ...
    if (/^https:\/\/meet\\.google\\.com\//i.test(hangoutLink)) return hangoutLink;
    ...
    const video = entryPoints.find(entry =>
      ...
      /^https:\/\/meet\\.google\\.com\//i.test(String(entry.uri || ''))
    );
