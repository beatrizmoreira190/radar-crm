...
const RADAR_REMINDER_HANDLER = 'sendRadarMeetingReminders_';

function authorizeRadar() {
  const now = new Date();
  CalendarApp.getDefaultCalendar().getEvents(now, new Date(now.getTime() + 60000));
  MailApp.getRemainingDailyQuota();
  ensureRadarReminderTrigger_();
}

function ensureRadarReminderTrigger_() {
  const matching = ScriptApp.getProjectTriggers()
    .filter(trigger => trigger.getHandlerFunction() === RADAR_REMINDER_HANDLER);
  if (!matching.length) {
    ScriptApp.newTrigger(RADAR_REMINDER_HANDLER).timeBased().everyHours(1).create();
    return 1;
  }
...
function radarReminderStatus_() {
  const triggerCount = ScriptApp.getProjectTriggers()
    .filter(trigger => trigger.getHandlerFunction() === RADAR_REMINDER_HANDLER).length;
  return {
...
function sendRadarMeetingReminders_() {
  const calendar = CalendarApp.getDefaultCalendar();
  const now = new Date();
  const windowStart = new Date(now.getTime() + 60 * 60 * 1000);
  const windowEnd = new Date(now.getTime() + 24 * 60 * 60 * 1000);
  const timezone = calendar.getTimeZone() || Session.getScriptTimeZone() || 'America/Sao_Paulo';
  const events = calendar.getEvents(windowStart, windowEnd);

  events.forEach(event => {
    const meetingId = event.getTag('radarMeetingId');
    if (!meetingId || event.getTag('radarReminderSentAt')) return;

    const guests = Array.from(new Set(
      event.getGuestList()
        .map(guest => String(guest.getEmail() || '').trim())
        .filter(email => email && email.indexOf('@') > 0)
    ));

    if (!guests.length) return;
    if (MailApp.getRemainingDailyQuota() < guests.length) return;

    const when = Utilities.formatDate(event.getStartTime(), timezone, "dd/MM/yyyy 'às' HH:mm");
    const meetUrl = getGoogleMeetUrl_(event);
    const subject = 'Lembrete da nossa reunião — Radar';
    const bodyLines = [
      'Olá!',
      '',
      'Passando para lembrar que nossa reunião está agendada para ' + when + '.',
      ''
    ];
    if (meetUrl) {
      bodyLines.push('Acesse a reunião por aqui:');
      bodyLines.push(meetUrl);
      bodyLines.push('');
    }
    bodyLines.push(
      'Caso tenha algum imprevisto ou não consiga participar, pedimos que nos avise respondendo a este e-mail para que possamos reorganizar a agenda.',
      '',
      'Até breve!',
      'Equipe Comercial - Radar'
    );
    const body = bodyLines.join('\\n');

    const htmlBody =
      '<p>Olá!</p>' +
      '<p>Passando para lembrar que nossa reunião está agendada para <strong>' + escapeHtml_(when) + '</strong>.</p>' +
      (meetUrl ? '<p><strong>Acesse a reunião por aqui:</strong><br><a href="' + escapeHtml_(meetUrl) + '">Entrar no Google Meet</a></p>' : '') +
      '<p>Caso tenha algum imprevisto ou não consiga participar, pedimos que nos avise respondendo a este e-mail para que possamos reorganizar a agenda.</p>' +
      '<p>Até breve!<br><strong>Equipe Comercial - Radar</strong></p>';
    ...
    event.setTag('radarReminderSentAt', new Date().toISOString());
  });
}

function extractMeetUrlFromText_(value) {
  ...
  let end = start;
  const stopChars = ' \\n\\r\\t<>"\'';
  while (end < text.length && stopChars.indexOf(text.charAt(end)) === -1) end++;
  return text.slice(start, end);
}

function getGoogleMeetUrl_(event) {
  try {
    const direct = extractMeetUrlFromText_([event.getLocation(), event.getDescription()].join('\\n'));
    if (direct) return direct;

    const endpoint = 'https://www.googleapis.com/calendar/v3/calendars/primary/events' +
      '?iCalUID=' + encodeURIComponent(event.getId()) +
      '&maxResults=1&singleEvents=true&fields=items(hangoutLink,conferenceData%2FentryPoints)';
    ...
    const hangoutLink = String(item && item.hangoutLink || '');
    if (hangoutLink.toLowerCase().indexOf('https://meet.google.com/') === 0) return hangoutLink;

    const entryPoints = item && item.conferenceData && Array.isArray(item.conferenceData.entryPoints)
      ? item.conferenceData.entryPoints
      : [];
    const video = entryPoints.find(entry =>
      entry && entry.entryPointType === 'video' &&
      String(entry.uri || '').toLowerCase().indexOf('https://meet.google.com/') === 0
    );
...
function doPost(e) {
  try {
    ...
    if (body.action === 'reminder_status') {
      const status = radarReminderStatus_();
      return json_({
        ok: true,
        account: Session.getEffectiveUser().getEmail() || '',
        remindersEnabled: status.enabled,
        triggerCount: status.triggerCount,
        remainingDailyQuota: status.remainingDailyQuota,
    ...
    if (body.action === 'update_event') {
      ...
      event.deleteTag('radarReminderSentAt');
