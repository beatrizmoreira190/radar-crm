...
function sheetXml(sheet){const rows=sheet.rows||[];const maxCols=Math.max(1,...rows.map(r=>r?.length||0),sheet.widths?.length||0);const lastRow=Math.max(1,rows.length);const dim=`A1:${colName(maxCols-1)}${lastRow}`;const widths=(sheet.widths||[]).map((w,i)=>`<col min="${i+1}" max="${i+1}" width="${Math.max(3,Math.min(60,Number(w)||10))}" customWidth="1"/>`).join('');const freeze=Math.max(0,Number(sheet.freezeRows)||0);const views=freeze?`<sheetViews><sheetView workbookViewId="0"><pane ySplit="${freeze}" topLeftCell="A${freeze+1}" activePane="bottomLeft" state="frozen"/><selection pane="bottomLeft" activeCell="A${freeze+1}" sqref="A${freeze+1}"/></sheetView></sheetViews>`:'<sheetViews><sheetView workbookViewId="0"/></sheetViews>';
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">${views}<dimension ref="${dim}"/>${widths?`<cols>${widths}</cols>`:''}<sheetData>${rowXml}</sheetData>${filter}${mergeXml}<pageMargins left="0.3" right="0.3" top="0.5" bottom="0.5" header="0.2" footer="0.2"/></worksheet>`;
}
...
function rootRels(){return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationshhip Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/><Relationshhip Id="rId2" Type="http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties" Target="docProps/core.xml"/><Relationship Id="rId3" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/extended-properties" Target="docProps/app.xml"/></Relationships>`}
