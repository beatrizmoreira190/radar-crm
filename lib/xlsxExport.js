...
function cellXml(input,row,col){const cell=normalizeCell(input);const ref=`${colName(col)}${row}`;const s=Number(cell.style)||0;const sv=s?` s="${s}"`:'';const v=cell.value;if(v===null||v===undefined||v==='')return s?`<c r="${ref}"${sv}/>`:'';
  if(cell.type==='date'||cell.type==='datetime'||v instanceof Date){const n=excelDate(v);return n===null?'':`<c r="${ref}"${sv} t="n"><v>${n}</v></c>`}
  if(cell.type==='number'||typeof v==='number'){const n=Number(v);return Number.isFinite(n)?`<c r="${ref}"${sv} t="n"><v>${n}</v></c>`:''}
...
function sheetXml(sheet){const rows=sheet.rows||[];const maxCols=Math.max(1,...rows.map(r=>r?.length||0),sheet.widths?.length||0);const lastRow=Math.max(1,rows.length);const dim=`A1:${colName(maxCols-1)}${lastRow}`;const widths=(sheet.widths||[]).map((w,i)=>`<col min="${i+1}" max="${i+1}" width="${Math.max(3,Math.min(60,Number(w)||10))}" customWidth="1"/>`).join('');const freeze=Math.max(0,Number(sheet.freezeRows)||0);const views=freeze?`<sheetViews><sheetView workbookViewId="0"><pane ySplit="${freeze}" topLeftCell="A${freeze+1}" activePane="bottomLeft" state="frozen"/><selection pane="bottomLeft" activeCell="A${freeze+1}" sqref="A${freeze+1}"/></sheetView></sheetViews>`:'<sheetViews><sheetView workbookViewId="0"/></sheetViews>';
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetPr><outlinePr summaryBelow="1" summaryRight="1"/><pageSetUpPr/></sheetPr><dimension ref="${dim}"/>${views}<sheetFormatPr baseColWidth="8" defaultRowHeight="15"/>${widths?`<cols>${widths}</cols>`:''}<sheetData>${rowXml}</sheetData>${filter}${mergeXml}<pageMargins left="0.3" right="0.3" top="0.5" bottom="0.5" header="0.2" footer="0.2"/></worksheet>`;
}
function stylesXml(){return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
<numFmts count="3"><numFmt numFmtId="164" formatCode="&quot;R$&quot; #,##0.00"/><numFmt numFmtId="165" formatCode="dd/mm/yyyy hh:mm"/><numFmt numFmtId="166" formatCode="dd/mm/yyyy"/></numFmts>
<fonts count="4">
<font><name val="Calibri"/><family val="2"/><color rgb="FF000000"/><sz val="11"/></font>
<font><name val="Calibri"/><family val="2"/><b val="1"/><color rgb="FF101828"/><sz val="16"/></font>
<font><name val="Calibri"/><family val="2"/><b val="1"/><color rgb="FF344054"/><sz val="11"/></font>
<font><name val="Calibri"/><family val="2"/><b val="1"/><color rgb="FFFFFFFF"/><sz val="11"/></font>
</fonts>
<fills count="5"><fill><patternFill/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FFFFF1F0"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FF111827"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFF2F4F7"/></patternFill></fill></fills>
<borders count="2"><border><left/><right/><top/><bottom/><diagonal/></border><border><left style="thin"><color rgb="FFD0D5DD"/></left><right style="thin"><color rgb="FFD0D5DD"/></right><top style="thin"><color rgb="FFD0D5DD"/></top><bottom style="thin"><color rgb="FFD0D5DD"/></bottom></border></borders>
<cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs>
<cellXfs count="12">
<xf numFmtId="0" fontId="0" fillId="0" borderId="0" pivotButton="0" quotePrefix="0" xfId="0"/>
<xf numFmtId="0" fontId="1" fillId="0" borderId="0" pivotButton="0" quotePrefix="0" xfId="0"/>
<xf numFmtId="0" fontId="2" fillId="2" borderId="1" pivotButton="0" quotePrefix="0" xfId="0"/>
<xf numFmtId="0" fontId="3" fillId="3" borderId="1" applyAlignment="1" pivotButton="0" quotePrefix="0" xfId="0"><alignment horizontal="center" vertical="center" wrapText="1"/></xf>
...
<xf numFmtId="0" fontId="0" fillId="0" borderId="0" applyAlignment="1" pivotButton="0" quotePrefix="0" xfId="0"><alignment vertical="top" wrapText="1"/></xf>
...
function workbookXml(sheets){return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><workbookPr/><workbookProtection/><bookViews><workbookView visibility="visible" minimized="0" showHorizontalScroll="1" showVerticalScroll="1" showSheetTabs="1" tabRatio="600" firstSheet="0" activeTab="0" autoFilterDateGrouping="1"/></bookViews><sheets>${sheets.map((s,i)=>`<sheet name="${xmlEscape(s.name.slice(0,31))}" sheetId="${i+1}" state="visible" r:id="rId${i+1}"/>`).join('')}</sheets><calcPr calcId="124519" fullCalcOnLoad="1"/></workbook>`}
