...
begin
  ...
  meeting_conversion as (
    select
      ...
      )::numeric,1) avg_days_first_contact_to_meeting
  ...
  select jsonb_build_object(
    ...
    'stage_aging',coalesce((
      select jsonb_agg(jsonb_build_object(
        'id',x.id,
        'name',x.name,
        'count',x.publisher_count,
        'avg_days',x.avg_days
      ) order by x.position)
      from (
        select s.id,s.name,s.position,
          count(p.id)::int as publisher_count,
          round(avg(extract(epoch from (now()-se.entered_at))/86400)::numeric,1) as avg_days
        from public.pipeline_stages s
    ...
    'meetings',jsonb_build_object(
      ...
      'by_presenter',coalesce((
        select jsonb_agg(jsonb_build_object(
          'user_id',x.presenter_user_id,
          'meetings',x.meetings,
          'completed',x.completed
        ) order by x.meetings desc)
        from (
          select m.presenter_user_id,
