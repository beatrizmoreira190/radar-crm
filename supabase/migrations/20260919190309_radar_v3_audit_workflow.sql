-- Radar v3 audit workflow.
-- Keeps radar_v2 official while adding review flags and human validation.

alter table public.radar_v3_simulations
  add column if not exists audit_flags jsonb not null default '[]'::jsonb;

create table if not exists public.radar_v3_reviews (
  organization_id uuid not null,
  publisher_id uuid not null references public.publishers(id) on delete cascade,
  review_status text not null default 'pending'
    check (review_status in ('pending','validated','needs_adjustment','later')),
  review_note text,
  reviewed_by uuid,
...
create index if not exists radar_v3_reviews_org_status_idx
  on public.radar_v3_reviews(organization_id,review_status);

alter table public.radar_v3_reviews enable row level security;

drop policy if exists radar_v3_reviews_select_manager on public.radar_v3_reviews;
create policy radar_v3_reviews_select_manager
on public.radar_v3_reviews
for select
to authenticated
using (
...
drop policy if exists radar_v3_reviews_insert_manager on public.radar_v3_reviews;
create policy radar_v3_reviews_insert_manager
on public.radar_v3_reviews
for insert
to authenticated
with check (
...
drop policy if exists radar_v3_reviews_update_manager on public.radar_v3_reviews;
create policy radar_v3_reviews_update_manager
on public.radar_v3_reviews
for update
to authenticated
using (
...
revoke all on table public.radar_v3_reviews from public,anon,authenticated;
grant select,insert,update on table public.radar_v3_reviews to authenticated;
  ...
  fitted as (
    select product,
      round(
        (
          case
            when mapped_count=0 then 0
            when mapped_count=1 then t1
            when mapped_count=2 then (t1*.85+t2*.10)/.95
            else t1*.85+t2*.10+t3*.05
          end
        ) *
        case
          when mapped_count=0 then 0
          when mapped_count=1 then 1.00
          when mapped_count=2 then .95 + .05*(strong_count::numeric/mapped_count::numeric)
          else .85 + .15*(strong_count::numeric/mapped_count::numeric)
        end
...
begin
  ...
  insert into public.radar_v3_simulations(
    ...
    top_products,product_fits,canonical_profiles,unknown_profiles,audit_flags,calculated_at
  )
  select
...
begin
  with x as (
    select s.*,coalesce(r.review_status,'pending') review_status
...
create or replace function public.crm_radar_v3_lab_rows_review(
  ...
  p_search text default null,p_status text default 'confirmed',p_review_status text default null
)
returns table(
  ...
  top_products jsonb,product_fits jsonb,unknown_profiles jsonb,audit_flags jsonb,
  review_status text,review_note text,reviewed_at timestamptz,calculated_at timestamptz
)
language plpgsql
stable
security invoker
...
begin
  ...
  select s.publisher_id,s.publisher_name,s.editorial_profile_status,s.editorial_profile_confidence,s.editorial_profile,
    ...
    s.top_products,s.product_fits,s.unknown_profiles,s.audit_flags,
    coalesce(r.review_status,'pending'),r.review_note,r.reviewed_at,s.calculated_at
  from public.radar_v3_simulations s
  left join public.radar_v3_reviews r
    on r.organization_id=s.organization_id and r.publisher_id=s.publisher_id
  where s.organization_id=p_organization_id
...
revoke all on function public.crm_radar_v3_lab_rows_review(uuid,integer,integer,text,text,text,text) from public,anon;
grant execute on function public.crm_radar_v3_lab_rows_review(uuid,integer,integer,text,text,text,text) to authenticated;
