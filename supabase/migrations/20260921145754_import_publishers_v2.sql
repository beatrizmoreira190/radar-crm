-- Importador de editoras v2: validação prévia, identidade segura e campos atuais do CRM.
-- Mantém o importador legado por compatibilidade; a interface nova usa as funções v2.

CREATE OR REPLACE FUNCTION private.crm_import_priority_value(p_value text)
 RETURNS text
 LANGUAGE sql
 IMMUTABLE
...
CREATE OR REPLACE FUNCTION private.crm_import_profile_confidence_value(p_value text)
 RETURNS text
 LANGUAGE sql
 IMMUTABLE
 SET search_path TO 'pg_catalog'
...
CREATE OR REPLACE FUNCTION private.crm_import_profile_status_value(p_value text)
 RETURNS text
 LANGUAGE sql
 IMMUTABLE
 SET search_path TO 'pg_catalog'
AS $function$
  select case lower(btrim(coalesce(p_value,'')))
    ...
    when 'not_identified' then 'not_identified'
    when 'nao identificado' then 'not_identified'
    when 'não identificado' then 'not_identified'
    when 'nao identificada' then 'not_identified'
    when 'não identificada' then 'not_identified'
    when 'review' then 'review'
    when 'revisar' then 'review'
    when 'revisao' then 'review'
    when 'revisão' then 'review'
    else null
  end
$function$

CREATE OR REPLACE FUNCTION private.crm_import_text_array(p_value jsonb)
 RETURNS text[]
 LANGUAGE sql
 IMMUTABLE
 SET search_path TO 'pg_catalog'
...
begin
  if role_name not in ('owner','admin') then
    raise exception 'Apenas administradores podem importar editoras' using errcode='42501';
  end if;
...
begin
  if role_name not in ('owner','admin') then
    raise exception 'Apenas administradores podem validar importações de editoras' using errcode='42501';
  end if;
