-- Garante source_ref unico por pessoa nos fluxos manual e XLSX.
-- Mantem os prefixos de origem para badges e classificacao na ficha.
...
begin
  ...
  for person in select value from jsonb_array_elements(legal_contacts) loop
    ...
    values(
      ...
      true,uid,uid,'manual_crm:legal:'||new_id::text||':'||md5(lower(person_name)||'|'||coalesce(person_email,''))
    );
  end loop;
  ...
  for person in select value from jsonb_array_elements(other_contacts) loop
    ...
    values(
      ...
      person_preferred_channel,
      nullif(btrim(coalesce(person->>'notes','')),''),
      true,uid,uid,'manual_crm:contact:'||new_id::text||':'||md5(lower(person_name)||'|'||coalesce(person_email,''))
    );
  end loop;
...
begin
  ...
  for item in select value from jsonb_array_elements(p_contacts) loop
    ...
    values(
      ...
      nullif(btrim(item->>'notes'),''),
      true,uid,uid,
      case when kind_value='legal' then 'xlsx_import:legal:' else 'xlsx_import:contact:' end||publisher_id_value::text||':'||md5(lower(person_name)||'|'||coalesce(person_email,''))
    );
